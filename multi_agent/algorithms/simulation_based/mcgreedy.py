    ...
    def run(self):
        ...
        for _ in tqdm(range(self.budget), desc=f'Agent {self.curr_agent_id} choosing node', leave=None):
            ...
            for u in tqdm(cim.inactive_nodes(self.graph), desc='Nodes examined', leave=None):
                ...
                marginal_gain = cim.simulation(self.graph, self.diff_model, agents=agents_copy, r=self.r)[agents_copy[self.curr_agent_id].name] - agents_copy[self.curr_agent_id].spread
