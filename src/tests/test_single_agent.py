...
import pandas as pd
from src.common.utils import read_adjacency_matrix, __calculate_similarities__
...
class MyTestCase(unittest.TestCase):
    ...
    def test_epinions_d_5(self):
        # Test epinions_d_5
        df = pd.DataFrame()
        g = read_adjacency_matrix('../data/epinions_d_5.txt')
        algo = ['group-pr']
        for a in algo:
            im = InfluenceMaximization(g, {'Agent_0': 40}, alg=a, diff_model='ic', inf_prob='uniform', r=1)
            seed = im.run()
    ...
    def test_twitter_d(self):
        # Test epinions_d_5
        df = pd.DataFrame()
        g = read_adjacency_matrix('../src/data/twitter-d.txt')
        algo = ['group-pr']
        for a in algo:
